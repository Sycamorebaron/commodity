...
from factor.cal_factor.factor_test import SingularVol
...
class _SingularVol(SingularVol):
    def trunc_data(self, _data):
        return SingularVol._trunc_data(self, _data)
...
def _singular_vol_cal(begin_date='2010-01-04', end_date='2021-02-28'):
    cal_factor = _SingularVol(
        factor_name='moment',
        begin_date=begin_date,
        end_date=end_date,
        init_cash=1000000,
    ...
    l_BV.to_excel(os.path.join(OUTPUT_DATA_PATH, '5D_BV.xlsx'))
    l_BV_sigma.to_excel(os.path.join(OUTPUT_DATA_PATH, '5D_BV_sigma.xlsx'))
    l_bollerslev_RSJ.to_excel(os.path.join(OUTPUT_DATA_PATH, '5D_bollerslev_RSJ.xlsx'))
