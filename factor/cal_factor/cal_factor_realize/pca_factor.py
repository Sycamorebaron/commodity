...
from factor.cal_factor.factor_test import PCAFactor
from utils.base_para import NORMAL_CONTRACT_INFO, local_data_path_5T, OUTPUT_DATA_PATH


pd.set_option('expand_frame_repr', False)

class _PCAFactor(PCAFactor):
    def trunc_data(self, _data):
        return PCAFactor._trunc_data(self, _data)


def pca_factor_cal(begin_date='2010-01-04', end_date='2021-02-28'):
...
def _pca_factor_cal(begin_date='2010-01-04', end_date='2021-02-28'):
    cal_factor = _PCAFactor(
        factor_name='moment',
        begin_date=begin_date,
        end_date=end_date,
        init_cash=1000000,
    ...
    l_d_first_com.to_excel(os.path.join(OUTPUT_DATA_PATH, '5D_d_first_com.xlsx'))
    l_d_sec_com.to_excel(os.path.join(OUTPUT_DATA_PATH, '5D_d_sec_com.xlsx'))
    l_first_com_range.to_excel(os.path.join(OUTPUT_DATA_PATH, '5D_first_com_range.xlsx'))
    l_sec_com_range.to_excel(os.path.join(OUTPUT_DATA_PATH, '5D_sec_com_range.xlsx'))
    l_d_first_com_std.to_excel(os.path.join(OUTPUT_DATA_PATH, '5D_d_first_com_std.xlsx'))
    l_d_sec_com_std.to_excel(os.path.join(OUTPUT_DATA_PATH, '5D_d_sec_com_std.xlsx'))
    l_first_explained_ratio.to_excel(os.path.join(OUTPUT_DATA_PATH, '5D_first_explained_ratio.xlsx'))
    l_sec_explained_ratio.to_excel(os.path.join(OUTPUT_DATA_PATH, '5D_sec_explained_ratio.xlsx'))
