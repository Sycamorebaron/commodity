...
class TradeCenter:
    ...
    @staticmethod
    def _inc(exchange, contract, num, price, now_date):
        """
        加仓，含加多和加空
        :param exchange:
        :param num:
        ...
        open_margin = \
            exchange.contract_dict[commodity].contract_unit * \
            exchange.contract_dict[commodity].init_margin_rate * price * abs(num)
        print('INC', now_date, contract, num, price)
        exchange.account.cash -= (
                open_margin + abs(num) * exchange.contract_dict[commodity].open_comm
        )
    ...
    @staticmethod
    def _dec(exchange, contract, num, price, now_date):
        """
        减仓，含减多和减空
        :param exchange:
        :param num:
            ...
            if hold_num == 0:
                ...
                if target_num[contract] > 0:
                    self._open(exchange=exchange, contract=contract, num=target_num[contract], price=price,
                               now_date=now_date)
                    trade_info.append(
                        {'now_date': now_date, 'trade_type': 'long_open', 'num': target_num[contract], 'price': price}
                    )
                # 开空
                    ...
                    self._open(exchange=exchange, contract=contract, num=target_num[contract], price=price,
                               now_date=now_date)
                    trade_info.append(
                        {'now_date': now_date, 'trade_type': 'short_open', 'num': target_num[contract], 'price': price}
                    )
            # 持有多头
            elif hold_num > 0:
                # 加多
                if target_num[contract] > hold_num:
                    self._inc(exchange=exchange, contract=contract, num=target_num[contract] - hold_num, price=price,
                              now_date=now_date)
                    trade_info.append(
                        {'now_date': now_date, 'trade_type': 'long_inc', 'num': target_num[contract] - hold_num,
                         'price': price}
                    )
                # 减多
                    ...
                    self._dec(exchange=exchange, contract=contract, num=target_num[contract] - hold_num, price=price,
                              now_date=now_date)
                    trade_info.append(
                        {'now_date': now_date, 'trade_type': 'long_dec', 'num': target_num[contract] - hold_num,
                         'price': price}
                    )
                # 平多
                elif target_num[contract] == 0:
                    ...
                    trade_info.append(
                        {'now_date': now_date, 'trade_type': 'long_close', 'num': -hold_num, 'price': price}
                    )
                # 平多开空
                elif target_num[contract] < 0:
                    self._close(exchange=exchange, contract=contract, price=price, now_date=now_date)
                    self._open(exchange=exchange, contract=contract, num=target_num[contract], price=price,
                               now_date=now_date)
                    trade_info.append(
                        {'now_date': now_date, 'trade_type': 'long_close_short_open',
            ...
            # 持有空头
                ...
                if target_num[contract] < hold_num:
                    self._inc(exchange=exchange, contract=contract, num=target_num[contract] - hold_num, price=price,
                              now_date=now_date)
                    trade_info.append(
                        {'now_date': now_date, 'trade_type': 'short_inc', 'num': target_num[contract] - hold_num,
                         'price': price}
                    )
                # 减空
                elif (target_num[contract] > hold_num) & (target_num[contract] < 0):
                    self._dec(exchange=exchange, contract=contract, num=target_num[contract] - hold_num, price=price,
                              now_date=now_date)
                    trade_info.append(
                        {'now_date': now_date, 'trade_type': 'short_dec', 'num': target_num[contract] - hold_num,
                         'price': price}
                    )
                # 平空
                elif target_num[contract] == 0:
                    self._close(exchange=exchange, contract=contract, price=price, now_date=now_date)
                    trade_info.append(
                        {'now_date': now_date, 'trade_type': 'short_close', 'num': target_num[contract] - hold_num,
                         'price': price}
                    )
                # 平空开多
                elif target_num[contract] > 0:
                    ...
                    self._open(exchange=exchange, contract=contract, num=target_num[contract], price=price,
                               now_date=now_date)
                    trade_info.append(
                        {'now_date': now_date, 'trade_type': 'short_close_long_open',
                         'num': target_num[contract] - hold_num, 'price': price}
                    )
